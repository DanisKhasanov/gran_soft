...
const AddAccountModal = () => {
  ...
  const handleAddAccount = async (
    accountType: 'StandardTradingAccount' | 'AppTradingAccount'
  ) => {
    setIsCreating(true);
    try {
      const newAccount = await dispatch(addAccount(accountType)).unwrap();
      handleClose();
      ...
      <div className="flex justify-end mt-6">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md ${
            isCreating ? 'cursor-not-allowed opacity-50' : 'hover:bg-gray-200'
          }`}
        >
